...
use WebWakeUpWdb\WithdrawalButton\Api\Webhook;
...
use WebWakeUpWdb\WithdrawalButton\Mail\WooAckEmail;
...
final class SettingsPage {
	 ...
	 * @var string
	 */
	private const NONCE = 'webwakeupwdb_save_settings';
	 ...
	 * @var string
	 */
	private const PREVIEW_NONCE = 'webwakeupwdb_preview_email';
	...
	public function render(): void {
		...
		$settings = wp_parse_args( (array) get_option( 'webwakeupwdb_settings', array() ), array( 'enabled' => false ) );
		$debug    = Audience::config();
		$saved    = isset( $_GET['webwakeupwdb_saved'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		echo '<input type="hidden" name="action" value="webwakeupwdb_save_settings" />';
	...
	private function render_complianz_section( array $settings ): void {
		if ( ! \WebWakeUpWdb\WithdrawalButton\Compat\ComplianzDocuments::is_complianz_active() ) {
			return;
		}
		...
		$companion      = \WebWakeUpWdb\WithdrawalButton\Compat\ComplianzDocuments::terms_companion_active();
	...
	private function complianz_preview( array $elements ): string {
		...
		$out  = '<details class="webwakeupwdb-clause" style="margin-top:.5em;"><summary>' . esc_html__( 'Preview what will be added', 'wwu-withdrawal-button' ) . '</summary>';
	...
	private function render_applicability_section( array $settings ): void {
		$app  = wp_parse_args(
			(array) get_option( 'webwakeupwdb_applicability', array() ),
			array(
				'mode'                 => 'eu_eea_only',
	...
	private function render_subscriptions_section( array $settings ): void {
		...
		echo '<tr><th scope="row">' . esc_html__( 'Renewal orders', 'wwu-withdrawal-button' )
			. ' <span class="webwakeupwdb-help" tabindex="0" title="' . esc_attr__( 'Leave OFF in almost all cases. A renewal continues the same contract — it has no fresh 14-day right.', 'wwu-withdrawal-button' ) . '" style="display:inline-block;width:16px;height:16px;line-height:16px;text-align:center;border-radius:50%;background:#e0e0e0;color:#333;font-size:11px;cursor:help;">?</span></th><td>';
		...
		echo '<tr><th scope="row">' . esc_html__( 'On withdrawal', 'wwu-withdrawal-button' )
			. ' <span class="webwakeupwdb-help" tabindex="0" title="' . esc_attr__( 'When a consumer withdraws from the initial order, optionally stop future renewals automatically.', 'wwu-withdrawal-button' ) . '" style="display:inline-block;width:16px;height:16px;line-height:16px;text-align:center;border-radius:50%;background:#e0e0e0;color:#333;font-size:11px;cursor:help;">?</span></th><td>';
		...
		echo '<details class="webwakeupwdb-clause"><summary><strong>' . esc_html__( 'Show example', 'wwu-withdrawal-button' ) . '</strong></summary>';
	 ...
	 * {@see \WebWakeUpWdb\WithdrawalButton\Platform\FluentCartAdapter::should_render()}.
	 *
	 * @param array $settings Current settings.
	 * @return void
	 */
	private function render_platforms_section( array $settings ): void {
		...
		$fc_native = \WebWakeUpWdb\WithdrawalButton\Platform\FluentCartAdapter::native_addon_active();
		...
		if ( ! $fc_active ) {
			...
			echo '<div class="wwu-ui-notice warning" style="margin:12px 0;max-width:860px;"><p style="margin:0 0 6px;"><strong>' . esc_html__( 'FluentCart now ships its own native withdrawal add-on (since FluentCart 1.4.2).', 'wwu-withdrawal-button' ) . '</strong></p><p style="margin:0;">' . wp_kses_post(
				sprintf(
					...
					'<code>webwakeupwdb_fluentcart_native_active</code>'
				)
			) . '</p></div>';
		}

		echo '<table class="form-table" role="presentation"><tbody>';
		echo '<tr><th scope="row"><label for="webwakeupwdb-fluentcart-mode">' . esc_html__( 'Withdrawal handling', 'wwu-withdrawal-button' ) . '</label>'
			. ' <span class="webwakeupwdb-help" tabindex="0" title="' . esc_attr__( "Auto (recommended): show our button on FluentCart orders, but step aside automatically if FluentCart's own withdrawal add-on is installed, so consumers never see two buttons.", 'wwu-withdrawal-button' ) . '" style="display:inline-block;width:16px;height:16px;line-height:16px;text-align:center;border-radius:50%;background:#e0e0e0;color:#333;font-size:11px;cursor:help;">?</span></th><td>';
		...
		echo '<select id="webwakeupwdb-fluentcart-mode" name="fluentcart_mode">';
		...
		if ( $fc_active && $fc_we_handle ) {
			...
			echo '<details class="webwakeupwdb-clause"><summary><strong>' . esc_html__( 'How to add it (3 steps)', 'wwu-withdrawal-button' ) . '</strong></summary>';
	 ...
	 * everywhere (Compliance page + [webwakeupwdb_info] shortcode), without the
	 * "sample text" disclaimer. Stored in the webwakeupwdb_clauses option per type+lang.
	 *
	...
	private function render_clauses_section( array $settings ): void {
		unset( $settings );
		$lang      = strtolower( substr( determine_locale(), 0, 2 ) );
		$overrides = (array) get_option( \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::OPTION, array() );
		...
		echo '<p class="description" style="max-width:60em;">' . esc_html(
			sprintf(
				...
				__( 'Customise the ready-to-paste legal clauses (language: %s). Leave a field empty to use the built-in sample template. When you type your own wording it replaces the default everywhere — the Compliance page and the [webwakeupwdb_info] shortcode — and the "sample text" note is dropped. This is your own text: have your lawyer review it.', 'wwu-withdrawal-button' ),
				strtoupper( $lang )
			)
		) . '</p>';
		echo '<table class="form-table" role="presentation"><tbody>';

		foreach ( \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::types() as $type ) {
			$label   = isset( $labels[ $type ] ) ? $labels[ $type ] : $type;
			$current = isset( $overrides[ $type ][ $lang ] ) ? (string) $overrides[ $type ][ $lang ] : '';
			$default = \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::default_text( $type, $lang );

			echo '<tr><th scope="row">' . esc_html( $label );
			if ( '' !== trim( $current ) ) {
				echo ' <span class="webwakeupwdb-badge webwakeupwdb-badge--ok" style="font-weight:400;">' . esc_html__( 'customised', 'wwu-withdrawal-button' ) . '</span>';
			}
			echo '</th><td>';
			echo '<textarea name="webwakeupwdb_clauses[' . esc_attr( $type ) . ']" rows="5" class="large-text" placeholder="' . esc_attr__( 'Leave empty to use the built-in sample template.', 'wwu-withdrawal-button' ) . '">' . esc_textarea( $current ) . '</textarea>';
	...
	private function render_receipt_section( array $settings ): void {
		$ts       = wp_parse_args( (array) get_option( 'webwakeupwdb_timestamp', array() ), array( 'provider' => 'none' ) );
		...
		echo '<tr id="webwakeupwdb-timestamp"><th scope="row">' . esc_html__( 'Trusted timestamp', 'wwu-withdrawal-button' ) . '</th><td>';
		...
		$preview_url = wp_nonce_url( admin_url( 'admin-post.php?action=webwakeupwdb_preview_email' ), self::PREVIEW_NONCE );
	...
	private function render_timestamp_reference(): void {
		...
		echo '<tr><th scope="row"></th><td>';
		echo '<details class="webwakeupwdb-clause"><summary><strong>' . esc_html__( 'Timestamp providers — which to choose', 'wwu-withdrawal-button' ) . '</strong></summary>';
	...
	private function render_appearance_section( array $settings ): void {
		...
		$vars = array(
			array( '--webwakeupwdb-accent', '#1a1f3a', __( 'Button background color', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-accent-text', '#ffffff', __( 'Button text color', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-accent-focus', '#1a1f3a', __( 'Button focus outline color', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-radius', '5px', __( 'Button corner radius', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-field-radius', '4px', __( 'Input/result corner radius', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-button-padding', '0.7em 1.4em', __( 'Button padding', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-button-font-size', '1rem', __( 'Button font size', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-button-font-weight', '600', __( 'Button font weight', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-button-border', '2px solid currentColor', __( 'Button border', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-form-max-width', '540px', __( 'Form max width', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-field-border', '#c3c4c7', __( 'Input border color', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-field-padding', '0.6em 0.7em', __( 'Input padding', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-label-weight', '600', __( 'Field label weight', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-note-color', '#666666', __( 'Helper/note text color', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-success-bg / -border / -text', '#edfaef / #46b450 / #1a4d24', __( 'Success message colors', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-error-bg / -border / -text', '#fcf0f1 / #d63638 / #8a1f21', __( 'Error message colors', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-notice-bg / -accent', '#f6f7f7 / #1a1f3a', __( 'Status-notice colors', 'wwu-withdrawal-button' ) ),
			array( '--webwakeupwdb-spacing', '1em', __( 'Vertical spacing rhythm', 'wwu-withdrawal-button' ) ),
		);

		// --- Reference: classes ---
		$classes = array(
			array( '.webwakeupwdb-button-wrap', __( 'Wrapper around the withdrawal button', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-button', __( 'Every button/CTA (withdraw, continue, confirm)', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-continue', __( 'The Step-1 "Continue" button', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-confirm', __( 'The statutory "Confirm withdrawal" button', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-window-note', __( 'The "X days left" note next to the button', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-form-wrap', __( 'The withdrawal form container', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-form-title', __( 'The form heading', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-field', __( 'Each field row (label + input)', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-actions', __( 'The actions row holding a button', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-step2 / .webwakeupwdb-step2-intro', __( 'The Step-2 confirmation block + its intro text', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-result.is-success / .is-error', __( 'The result message (success / error states)', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-status-notice', __( 'The "request status" notice on an order', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-notice', __( 'Generic inline notice', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-model-form (+ __fields)', __( 'The Annex I-B model form block', 'wwu-withdrawal-button' ) ),
			array( '.webwakeupwdb-info', __( 'The pre-contractual info block', 'wwu-withdrawal-button' ) ),
		);

		echo '<details class="webwakeupwdb-clause" style="margin-top:1em;"><summary><strong>' . esc_html__( 'CSS reference — variables & classes', 'wwu-withdrawal-button' ) . '</strong></summary>';
		...
		$examples = "/* Brand the button (quick way) */\n:root {\n    --webwakeupwdb-accent: #f49619;\n    --webwakeupwdb-accent-text: #ffffff;\n    --webwakeupwdb-radius: 8px;\n}\n\n/* Full control on the confirm button */\n.webwakeupwdb-confirm {\n    text-transform: uppercase;\n    letter-spacing: .5px;\n}\n\n/* Make the form wider */\n.webwakeupwdb-form-wrap { max-width: 680px; }";
		echo '<pre class="webwakeupwdb-inspector__snapshot" style="max-width:860px;">' . esc_html( $examples ) . '</pre>';
	...
	private function render_exemptions_section(): void {
		$exclusions = (array) get_option( 'webwakeupwdb_exclusions', array() );
		...
		$main_opt   = (array) get_option( 'webwakeupwdb_settings', array() );
		...
		foreach ( \WebWakeUpWdb\WithdrawalButton\Domain\ExceptionTypes::all() as $id => $def ) {
			$by_group[ \WebWakeUpWdb\WithdrawalButton\Domain\ExceptionTypes::group( (string) $id ) ][ (string) $id ] = $def;
		}
		...
		foreach ( $groups as $gkey => $g ) {
			...
			echo '<details class="wwu-ui-accordion" id="webwakeupwdb-group-' . esc_attr( $gkey ) . '"' . ( $g[2] ? ' open' : '' ) . '>';
	...
	private function render_reason_block( string $id, array $def, array $by_reason, array $legacy_p, array $legacy_c ): void {
		...
		echo '<div class="webwakeupwdb-reason" id="webwakeupwdb-reason-' . esc_attr( $id ) . '" style="border:1px solid #e0e0e0;border-radius:4px;padding:12px 14px;margin:0 0 10px;background:#fff;">';
		...
		if ( '' !== $hint ) {
			echo ' <span class="webwakeupwdb-help" tabindex="0" title="' . esc_attr( $hint ) . '" style="display:inline-block;width:16px;height:16px;line-height:16px;text-align:center;border-radius:50%;background:#e0e0e0;color:#333;font-size:11px;cursor:help;">?</span>';
		}
		...
		if ( ! empty( $def['conditional'] ) ) {
			...
			$consent_text = \WebWakeUpWdb\WithdrawalButton\Domain\ConsentText::for_reason( $id );
			$email_html   = \WebWakeUpWdb\WithdrawalButton\Mail\ExemptionConfirmation::preview_html( $id );
			if ( '' !== $consent_text ) {
	...
	private function render_exemptions_status( array $by_reason, array $legacy_p, array $legacy_c, bool $capture_ip ): void {
		...
		$last_purge = (string) get_option( 'webwakeupwdb_consent_last_purge', '' );
	...
	private function render_exemptions_helper(): void {
		...
		foreach ( $cards as $card ) {
			...
			if ( '' !== $card[2] ) {
				echo '<p style="margin:6px 0 0;"><a href="#webwakeupwdb-group-' . esc_attr( $card[2] ) . '">' . esc_html__( '→ Go to the matching group below', 'wwu-withdrawal-button' ) . '</a></p>';
			}
			echo '</div></details>';
		}
		echo '</div>';
	...
	private function render_integrations_section(): void {
		$cfg     = Webhook::config();
		$has_key = '' !== $cfg['secret'];
		$rest    = esc_url( rest_url( WEBWAKEUPWDB_REST_NAMESPACE ) );
		...
		echo '<tr><th scope="row"><label for="webwakeupwdb-webhook-url">' . esc_html__( 'Endpoint URL', 'wwu-withdrawal-button' ) . '</label></th><td>';
		echo '<input type="url" id="webwakeupwdb-webhook-url" class="regular-text" name="webhook_url" value="' . esc_attr( $cfg['url'] ) . '" placeholder="https://hooks.example.com/withdrawal" />';
		...
		echo '<tr><th scope="row"><label for="webwakeupwdb-webhook-secret">' . esc_html__( 'Signing secret', 'wwu-withdrawal-button' ) . '</label></th><td>';
		if ( $has_key ) {
		...
		echo '<input type="text" id="webwakeupwdb-webhook-secret" class="regular-text" name="webhook_secret" value="" autocomplete="off" placeholder="' . esc_attr__( 'Paste your own secret, or tick “Generate” below', 'wwu-withdrawal-button' ) . '" />';
	...
	public function handle_save(): void {
		...
		$clauses        = (array) get_option( \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::OPTION, array() );
		$posted_clauses = ( isset( $_POST['webwakeupwdb_clauses'] ) && is_array( $_POST['webwakeupwdb_clauses'] ) ) ? (array) wp_unslash( $_POST['webwakeupwdb_clauses'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- each field is sanitized below.
		foreach ( \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::types() as $clause_type ) {
		...
		update_option( \WebWakeUpWdb\WithdrawalButton\Legal\ClauseLibrary::OPTION, $clauses, false );
		...
		update_option( 'webwakeupwdb_applicability', $applicability );
		...
		foreach ( $posted_exempt as $reason => $sets ) {
			...
			if ( ! \WebWakeUpWdb\WithdrawalButton\Domain\ExceptionTypes::exists( $reason ) ) {
				continue;
			}
		...
		\WebWakeUpWdb\WithdrawalButton\Frontend\WooBlockCheckoutConsent::flush_cache();

		// Timestamp provider.
		$timestamp = (array) get_option( 'webwakeupwdb_timestamp', array() );
		...
		if ( '' !== $rfc3161['endpoint'] && ! \WebWakeUpWdb\WithdrawalButton\Security\OutboundUrlGuard::is_safe_url( $rfc3161['endpoint'] ) ) {
			$rfc3161['endpoint'] = '';
		}
		...
		update_option( 'webwakeupwdb_timestamp', $timestamp );
		...
		$webhook            = (array) get_option( 'webwakeupwdb_webhook', array() );
		...
		update_option( 'webwakeupwdb_webhook', $webhook );
		...
		update_option( 'webwakeupwdb_debug', $debug );
		...
		\WebWakeUpWdb\WithdrawalButton\Compat\ComplianzDocuments::flush();

		wp_safe_redirect(
			add_query_arg(
				array(
					'page'         => AdminController::SETTINGS_SLUG,
					'webwakeupwdb_saved' => '1',
				),
				admin_url( 'admin.php' )
			)
		);
