...
- **Last updated:** 2026-06-15 (new **Subscription-aware withdrawal SPEC** — `docs/specs/wwu-wb-subscriptions-withdrawal-SPEC.md`, design-only, awaiting go-ahead: button on the initial order only, suppressed on renewals; per-platform renewal detection + optional auto-cancel; recon-grounded. On top of alpha.37 FluentCart merge-tag.)
...
- [SPEC — Subscription-aware withdrawal (EU 14-day)](docs/specs/wwu-wb-subscriptions-withdrawal-SPEC.md) — **design, awaiting go-ahead** (2026-06-15): make the withdrawal button subscription-aware — button on the INITIAL order only, **suppressed on renewals** (one `renewal_order` gate in `ApplicabilityResolver` fed by a new `NormalizedOrder::$is_renewal`, auto-covering all 8 surfaces; + a `Shortcodes::form()` guard). Per-platform renewal detection (WC Subscriptions `wcs_order_contains_renewal`; FluentCart `Subscription.parent_order_id`; EDD Recurring `edd_subscription` status) via an optional `SubscriptionAware` adapter interface; optional auto-cancel-on-withdrawal (default off) + Requests reminder (cancel + Art. 14(3) pro-rata). Distinct from the NA click-to-cancel module. Grounded in a 2-agent recon (codebase + EUR-Lex/Cod. Consumo art. 9/13/14(3) = 52/56/57).
