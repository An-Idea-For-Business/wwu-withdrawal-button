...
- **Fix — `on-hold` is eligible only when paid.** `ApplicabilityResolver::is_eligible_status()` had `on-hold` in the unconditional allowlist, but in WooCommerce `on-hold` is the canonical *awaiting-payment* status for offline gateways (bank transfer / cheque) — contradicting the method's own intent ("never show on unpaid orders") and diverging from FluentCart, which never surfaces an unpaid order. `on-hold` is now eligible only when a payment date was actually recorded (`NormalizedOrder->paid !== null`), so an unpaid bank-transfer order is hidden while a *paid* order held for manual review still shows. `src/Domain/ApplicabilityResolver.php`.
- **Hardening — applicability is now enforced server-side.** `WithdrawalService::submit_statement()` (the single chokepoint both the REST route and the no-JS `admin-post` flow call) now refuses to start a withdrawal when `ApplicabilityResolver::decide()->show` is false, returning a `webwakeupwdb_not_withdrawable` `WP_Error` (403). Previously the applicability decision gated only the UI (which orders render the button); a stale page or a crafted POST could still record a withdrawal for an ineligible order (unpaid, out-of-scope, B2B, renewal, Art. 59-exempt). The gate is filter-aware (honours `webwakeupwdb_applicability_decision`). `src/Domain/WithdrawalService.php`, `src/REST/Routes/WithdrawalRoute.php`, `src/Frontend/NoScriptFlow.php`.
- **Tests.** `suite_applicability` gains `onhold_unpaid_hidden` + `onhold_paid_eligible`; the `fake_order` helper gains an optional `$paid` argument. Verified out-of-band with a standalone reflection harness over `is_eligible_status()` (on-hold-unpaid / pending / cancelled / refunded hidden; processing / completed / paid / on-hold-paid shown).
- **Fix — bundled translations load on WP 5.8–6.0** (folded in from PR #57). 1.2.4 removed `load_plugin_textdomain()` assuming WP 4.6+ JIT-loads a plugin's bundled `/languages` catalogue, but that holds for a plugin's OWN bundled translations only from **WP 6.1+** (`WP_Textdomain_Registry`); on the declared floor (WP 5.8–6.0) the bundled `it/de/es/fr/sv` `.mo` would not load, so those users saw English. Re-added an `init`-hooked `load_plugin_textdomain( WEBWAKEUPWDB_TEXT_DOMAIN, false, dirname( WEBWAKEUPWDB_PLUGIN_BASENAME ) . '/languages' )`; harmless on WP 6.1+, and WordPress.org language packs still take precedence once generated. (translate.wordpress.org showing 0% is separate and expected — GlotPress does not import bundled `.po`, and language packs require ~90% human-reviewed strings; unreviewed machine translations are not accepted per Polyglots policy.) `wwu-withdrawal-button.php`.
...
- **i18n — all 5 locales re-translated to ~99–100%.** Regenerating the `.pot` for this feature surfaced a **pre-existing debt**: ~110 strings per locale (mostly the 1.3.x Legal Documents / policy + trusted-timestamping admin UI) were untranslated — and shipped untranslated in the **live 1.3.2** (an Italian merchant saw those screens in English). Closed it: this feature's 7 new strings **plus** the full backlog are now translated across it/de/es/fr/sv via `wwu-tools/wwu-i18n.php` (extract → merge → 5 parallel translators → apply → compile), placeholder-parity validated. The handful still English are intentional brand / dev-tool labels (Dashboard, Debug, Debug Inspector, Snapshot, FluentCart, IP, WWU Withdrawal Button). Result: de 99% · es 100% · fr 100% · it 99% · sv 100%.
